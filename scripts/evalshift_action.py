...
class ActionConfig:
    ...
    @classmethod
    def from_env(cls, env: dict[str, str] | None = None) -> ActionConfig:
...
def _input(env: dict[str, str], name: str, default: str = "") -> str:
    return env.get(f"INPUT_{name}", default).strip()


def _bool_input(env: dict[str, str], name: str, default: bool) -> bool:
    raw = _input(env, name, "true" if default else "false").lower()
    return raw in {"1", "true", "yes", "on"}


def detect_context(env: dict[str, str], branch: str, base_branch: str) -> GitHubContext:
    ...
    pull_number = event.get("number") if isinstance(event.get("number"), int) else None
...
def evaluate_gating(diff: dict[str, Any] | None, fail_on: str) -> GatingResult:
    ...
    aggregate = diff.get("aggregate_delta") if isinstance(diff.get("aggregate_delta"), dict) else {}
    regression_count = int(aggregate.get("regressions") or 0)
    slices = diff.get("per_slice_deltas") if isinstance(diff.get("per_slice_deltas"), list) else []
    slice_regressions = [
        item
        for item in slices
